...
当可用宽度不足时，Ribbon 会将全部功能组折叠为下拉菜单；全部功能组折叠后仍然
溢出时，内容区两端会按需显示悬浮滚动按钮。原生横向滚动条始终隐藏，滚动到最左或
最右边界时，对应方向的按钮也会隐藏。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `type` | 否 | `button`、`splitButton` 或 `dropdown`，缺省为 `button` |
...
嵌套；功能组因宽度不足折叠时，也会保留这些组件，而不是降级为仅命令列表。

### 自定义 HTML、CSS 与 JavaScript

`menu.custom` 在隔离的 `iframe sandbox="allow-scripts"` 中渲染。自定义脚本不能访问
Workbench DOM、Cookie、存储或扩展 Node.js API；网络连接默认被 CSP 禁止。HTML、
CSS 和 JavaScript 均为字符串，弹层尺寸可在限定范围内配置：
...
```jsonc
{
  ...
  "menu": {
    "custom": {
      "width": 320,
      "height": 180,
      "html": "<button id='red'>红色</button><button id='blue'>蓝色</button>",
      "css": "body { display: flex; gap: 8px; padding: 12px } button { min-width: 72px }",
      "javascript": "const api = acquireRibbonApi(); document.querySelector('#red').onclick = () => api.executeCommand('publisher.extension.setColor', 'red'); document.querySelector('#blue').onclick = () => api.executeCommand('publisher.extension.setColor', 'blue');",
      "commands": ["publisher.extension.setColor"]
    }
  }
}
```

自定义脚本可调用 `acquireRibbonApi()`，返回冻结对象：

- `executeCommand(command, ...arguments)`：仅当命令 ID 已列入当前 `custom.commands` 时执行。
- `close()`：关闭当前下拉菜单。

`commands` 中的命令仍应登记在 `contributes.commands`。宿主会忽略未列入白名单的命令，
因此自定义内容只应申请实际需要的最小命令集合。`menu.items` 与 `menu.custom` 可以同时
存在，此时先显示声明式 Ribbon 组件，再显示自定义区域。
