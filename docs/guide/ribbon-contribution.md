...
会隐藏。功能组被折叠后，分组位置只剩一个下拉触发按钮，点击它会弹出**该分组在
未折叠时的原样内容**（同一组按钮节点、横向 6 行网格、`full` / `half` / `third`
三种尺寸与 `splitButton` 双层结构都保持不变），无需任何额外声明。
...
::: tip 折叠分组下拉不走 `layout`
`menu.layout` 只对**按钮级**下拉（`splitButton` / 单体 `dropdown` 的 `menu` 字段）
生效。**分组级**下拉——也就是功能组因窗口宽度不足被折叠后，点击分组下拉按钮弹出
的内容——不读取 `layout`，永远直接复用该分组在 ribbon 主区域里已经渲染好的
`.ribbon-items` 节点：同一组按钮、横向 6 行网格、`full` / `half` / `third` 三种尺寸、
`splitButton` 双层结构全部保持原样，不需要任何额外声明。内置 `File / View / Help`
标签的折叠分组下拉也走同一条路径，与扩展标签视觉一致。
:::
...
- 分组被折叠后点击下拉触发按钮，弹出的就是该分组未折叠时的原样内容（同一组按钮、
  横向网格、`full` / `half` / `third` 尺寸与 `splitButton` 双层结构保持不变）；
  关闭下拉后分组 DOM 恢复正常，再次展开 Ribbon 时按钮行为正常。
- `splitButton` / `dropdown` 声明了 `menu.layout: 'panel'` 时，面板已正确显示标题
  （若声明）、网格按 `columns` 排布、`items` 与 `custom` 共存时顺序正确；切换标签或
  点击其它按钮后面板能自动关闭，多个面板之间切换不会出现「旧的收了、新的没弹」。
...
顶部出现标题栏与关闭按钮。注意 `layout` 只对**按钮级**下拉（`splitButton` /
`dropdown` 的 `menu`）生效，**分组级**折叠下拉不走这条路径。

### 折叠分组下拉显示成竖排菜单或右键菜单

分组被折叠后点击下拉触发按钮，弹出的应该是该分组未折叠时的原样内容（横向 6 行
网格）。如果看到的是竖排命令列表或 VS Code 原生右键菜单，说明运行的不是最新实现：
旧版本里扩展分组走竖排菜单、内置 `File / View / Help` 分组走 `IContextMenuService`
原生右键菜单，两者都已统一改为复用 `.ribbon-items` 原样 DOM。重新构建并确认补丁
`patches/219-feature-ribbon-part.patch` 已更新到包含 `showFoldedGroupDropdown` 的
版本。
